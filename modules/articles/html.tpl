...
function addVal(){
	var page_id = {PAGE_ID};
	$('#QuestDiv').html('');
	$.post("modules/articles/add_art.php", {page_id:page_id},
			function(data){
				//alert(data);
...
function edtVal(id){
	$.post("modules/articles/item.php", {id:id}, 
		function(data){
			//alert(data);
			var obj = jQuery.parseJSON(data);
			if(obj.result=='OK'){
...
function addQuestion() {
	...
	$.post("modules/articles/add_q.php", {art_id:art_id},
			function(data){
				//alert(data);
				var obj = jQuery.parseJSON(data);
				if(obj.result=='OK'){
...
function AddAnswer(q_id) {
	$.post("modules/articles/add_a.php", {q_id:q_id},
			function(data){
				//alert(data);
				var obj = jQuery.parseJSON(data);
				if(obj.result=='OK'){
...
function DelQ(id){
	swal({
			...
			function(){
				$.post("modules/articles/del_q.php", {id:id},
						function(data){
							var obj = jQuery.parseJSON(data);
							if(obj.result=='OK'){
								swal("Успешно", "Запись удалена.", "success");
								$("#DivQforDel"+id).remove();
								//setTimeout('window.location.reload()', 2000);
							}
							else{
								swal("Ошибка Сервера!", "Объект ненайден !", "error");
...
function DelA(id){
	swal({
			...
			function(){
				$.post("modules/articles/del_a.php", {id:id},
						function(data){
							var obj = jQuery.parseJSON(data);
							if(obj.result=='OK'){
								swal("Успешно", "Запись удалена.", "success");
								$("#DivAforDel"+id).remove();
								//setTimeout('window.location.reload()', 2000);
							}
							else{
								swal("Ошибка Сервера!", "Объект ненайден !", "error");
...
function getErrs(id){
    $.post("modules/articles/get_errs.php", {id:id},
            function(data){
                //alert(data);
                var obj = jQuery.parseJSON(data);
                if(obj.result=='OK'){
                    if(obj.count>0){
                        var err_arr = jQuery.makeArray(obj.err);
                        for(i=0;i<err_arr.length;i++){
                            $("#errs"+obj.err[i]).attr('checked', true);
                        }
                    }
                }
                else{
